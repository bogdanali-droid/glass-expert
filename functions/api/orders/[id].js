/**
 * GET    /api/orders/:id — comandă completă
 * PUT    /api/orders/:id — update comandă
 * DELETE /api/orders/:id — ștergere comandă
 */

import { json, err, now, safeJsonParse } from '../_helpers.js';
...
export async function onRequestGet(context) {
    ...
    try {
        ...
        return json({ order: { ...order, lines: safeJsonParse(order.lines, []) } });
    } catch (e) {
...
export async function onRequestPut(context) {
    ...
    try {
        const existing = await DB
            ...
            .bind(id)
            .first();
        if (!existing) return err('Comanda nu există', 404);

        const allowed = [
            'client_id', 'client_cod', 'client_alias', 'quote_cod',
            'date_created', 'date_delivery', 'stage', 'lot_fabrica',
            'discount', 'priority', 'notes', 'notes_interne',
        ];
...
export async function onRequestDelete(context) {
    ...
    try {
        const order = await DB
            .prepare('SELECT id, cod FROM orders WHERE id = ?')
            .bind(id)
            .first();
        if (!order) return err('Comanda nu există', 404);
        ...
        return json({ success: true, deleted: { id: order.id, cod: order.cod } });
    } catch (e) {
        return err('Database error: ' + e.message, 500);
    }
}
